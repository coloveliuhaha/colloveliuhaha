...
import cn.hollis.turbo.stream.param.Message;
    ...
    protected ChainProcessResponse doPostExecute(ChainProcessRequest chainProcessRequest, ChainOperateTypeEnum chainOperateTypeEnum,
        ...
        return handle(chainProcessRequest, request -> {
            ...
            ChainOperateInfo chainOperateInfo = chainOperateInfoService.queryByOutBizId
                    (
                        chainProcessRequest.getBizId(),
                        chainProcessRequest.getBizType(),
            ...
            var operateInfoId = chainOperateInfoService.insertInfo
                    (
                            chainType(),
                            chainProcessRequest.getBizId(),
                            chainProcessRequest.getBizType(),
                            chainOperateTypeEnum.name(),
                            JSON.toJSONString(chainProcessRequest),
                            chainProcessRequest.getIdentifier()
                    );
            //核心逻辑执行
                ...
                scheduler.schedule(() -> {
                    try {
                        ...
                        ChainProcessResponse<ChainResultData> queryChainResult = queryChainResult(
                                new ChainQueryRequest
                                        (
                                                chainProcessRequest.getIdentifier(),
                                                operateInfoId.toString()
                                        ));
...
//                                // TODO Spring Stream 传送给MQ异步解耦
//                                this.sendMsg(operateInfo, queryChainResult.getData());
                                // 更新上链记录
                                chainOperateInfoService.updateResult
                                        (
                                                operateInfoId,
