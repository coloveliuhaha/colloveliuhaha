...
public class WenChangChainServiceImpl extends AbstractChainService {
    ...
    public ChainProcessResponse<ChainCreateData> createAddr(ChainProcessRequest chainProcessRequest) {
        ...
        // 三方传输信息，获取上链信息
        ChainProcessResponse response = doPostExecute
                (
                        chainProcessRequest,
                        ChainOperateTypeEnum.USER_CREATE,
                        chainRequest -> chainRequest.build(body, path, signature, wenChangChainConfiguration.host(), currentTime)
                );
